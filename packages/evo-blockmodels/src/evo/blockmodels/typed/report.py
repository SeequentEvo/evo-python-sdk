...
class Report:
    ...
    def _get_blocksync_url(self, result_id: UUID | None = None) -> str:
        """Generate the BlockSync URL for this report.

        :param result_id: Optional result ID to link to a specific result.
        :return: The BlockSync URL.
        """
        environment = self._context.get_environment()
        hub = environment.hub_url

        # Determine environment (integration vs production)
        if "int" in hub or "integration" in hub or "qa" in hub:
            blocksync_base = "https://blocksync.integration.seequent.com"
        else:
            blocksync_base = "https://blocksync.seequent.com"

        # Extract hub code from hub_url (e.g., "350mt" from "https://350mt.api.integration.seequent.com")
        hub_code = hub.split("//")[1].split(".")[0] if "//" in hub else "workspace"

        url = (
            f"{blocksync_base}/{environment.org_id}/{hub_code}/{environment.workspace_id}"
            f"/blockmodel/{self._block_model_uuid}/reports/{self.id}"
        )
    ...
    @property
    def blocksync_url(self) -> str:
        """The URL to view this report in BlockSync."""
        return self._get_blocksync_url()
    ...
    def _repr_html_(self) -> str:
        ...
        block_model_display = f"{self._block_model_name} ({self._block_model_uuid})" if self._block_model_name else str(
            self._block_model_uuid)
        rows = [
