...
class TestReportResult(TestWithConnector):
    ...
    def test_repr_html(self) -> None:
        """Test HTML representation of report result."""
        result = ReportResult(
            result_uuid=RESULT_UUID,
            report_specification_uuid=RS_UUID,
            block_model_uuid=BM_UUID,
            ...
            categories=[],
            columns=[{"label": "Au Grade", "unit_id": "g/t"}],
            result_sets=[{"cutoff_value": None, "rows": [{"categories": [], "values": [2.5]}]}],
        )
...
class TestReport(TestWithConnector, TestWithStorage):
    ...
    def test_blocksync_url_integration(self) -> None:
        """Test BlockSync URL generation for integration environment."""
        spec = self._mock_specification()
        context = StaticContext.from_environment(self.environment, self.connector, self.cache)
        report = Report(context, BM_UUID, spec)

        # Mock the environment hub_url for the test by testing the URL generation logic
        # The default test environment uses unittest.localhost, but we can verify the URL structure
        url = report.blocksync_url
        # Should contain the report ID, block model ID, org ID, and workspace ID
        self.assertIn(str(BM_UUID), url)
        self.assertIn(str(RS_UUID), url)
        self.assertIn(str(self.environment.org_id), url)
        self.assertIn(str(self.environment.workspace_id), url)
        self.assertIn("blocksync", url)

    def test_blocksync_url_with_result_id(self) -> None:
        """Test BlockSync URL generation with result ID."""
        spec = self._mock_specification()
        context = StaticContext.from_environment(self.environment, self.connector, self.cache)
        report = Report(context, BM_UUID, spec)

        # Test URL with result_id
        result_id = uuid.uuid4()
        url = report._get_blocksync_url(result_id=result_id)
        self.assertIn(f"result_id={result_id}", url)

    def test_repr_html(self) -> None:
        """Test HTML representation contains BlockSync link."""
        spec = self._mock_specification()
        context = StaticContext.from_environment(self.environment, self.connector, self.cache)
        report = Report(context, BM_UUID, spec)
        ...
        self.assertIn("blocksync", html.lower())
        self.assertIn("Open in BlockSync", html)
        self.assertIn("Domain", html)  # Category label
        self.assertIn("Au Grade", html)  # Column label
