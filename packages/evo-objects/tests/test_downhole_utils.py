...
class TestDownholeUtils(unittest.TestCase):
    def _chunks(self, values, hole_indices=None):
        return hole_chunks_from_ids(pd.Series(values), hole_indices=hole_indices)
    ...
    def test_varying_hole_counts_are_preserved(self):
        chunks = self._chunks(["A", "B", "B", "B", "C", "C"])
        self.assertListEqual(
            chunks.to_dict("records"),
            [
    ...
    def test_mapping_ids_absent_from_data_do_not_emit_chunks(self):
        chunks = self._chunks(["A", "A"], {"A": 1, "B": 42})
        self.assertListEqual(chunks.to_dict("records"), [{"hole_index": 1, "offset": 0, "count": 2}])

    def test_id_absent_from_hole_indices_raises(self):
        with self.assertRaises(ValueError):
            self._chunks(["A", "C"], {"A": 1, "B": 42})

    def test_empty_input_emits_no_chunks(self):
        chunks = self._chunks([], {"A": 1, "B": 42})
        self.assertListEqual(chunks.to_dict("records"), [])

    def test_hole_index_dtype_is_int32(self):
        self.assertEqual(str(self._chunks(["A"])["hole_index"].dtype), "int32")
    ...
    def test_fifty_holes_with_two_hundred_rows_each_round_trip(self):
        hole_ids = [f"H{index:02d}" for index in range(50)]
        values = [hole_id for hole_id in hole_ids for _ in range(200)]
    ...
    def test_creation_keys_are_sorted_by_hole_id(self):
        chunks = self._chunks(["M", "M", "Z"])
        self.assertListEqual(
            chunks.to_dict("records"),
            [
                {"hole_index": 0, "offset": 0, "count": 2},
                {"hole_index": 1, "offset": 2, "count": 1},
            ],
        )

    def test_chunks_use_dense_creation_keys_and_round_trip(self):
        values = pd.Series(["M", "M", "Z"])
        chunks = hole_chunks_from_ids(values)
        self.assertListEqual(chunks["hole_index"].tolist(), [0, 1])
        self.assertListEqual(chunks["count"].tolist(), [2, 1])
        self.assertListEqual(expand_hole_index(chunks, len(values)).tolist(), [0, 0, 1])

    def test_chunks_preserve_explicit_sparse_lookup_keys(self):
        chunks = self._chunks(["M", "M", "Z"], {"Z": 1, "M": 42})
        self.assertListEqual(
            chunks.to_dict("records"),
            [{"hole_index": 42, "offset": 0, "count": 2}, {"hole_index": 1, "offset": 2, "count": 1}],
        )

    def test_non_contiguous_and_unknown_ids_raise(self):
        with self.assertRaises(ValueError):
            hole_chunks_from_ids(pd.Series(["A", "B", "A"]), hole_indices={"A": 1, "B": 42})
        with self.assertRaises(ValueError):
            hole_chunks_from_ids(pd.Series(["C"]), hole_indices={"A": 1, "B": 42})

    def test_empty_input_uses_schema_dtypes(self):
        chunks = hole_chunks_from_ids(pd.Series([], dtype="string"), hole_indices={"A": 1, "B": 42})
        self.assertListEqual(chunks["hole_index"].tolist(), [])
        self.assertListEqual(chunks["offset"].tolist(), [])
        self.assertListEqual(chunks["count"].tolist(), [])
