...
from .base import ConstructableObject
...
class BlockModel(BaseSpatialObject, ConstructableObject[BlockModelData]):
    ...
    def blocksync_url(self) -> str:
        """The URL to view this block model in BlockSync Portal.

        :return: The BlockSync Portal URL.
        """
        from evo.blockmodels.urls import get_blocksync_block_model_url_from_environment

        return get_blocksync_block_model_url_from_environment(
            environment=self._obj.metadata.environment,
            block_model_id=self.block_model_uuid,
        )

    def _metadata_repr_html_(self) -> str:
        """Return an HTML representation of common metadata for Jupyter notebooks.

        Overrides the base implementation to include BlockSync link.
        """
        doc = self.as_dict()
        ...
        # Build title links including BlockSync
        title_links = [
            ("Portal", self.portal_url),
            ("Viewer", self.viewer_url),
            ("BlockSync", self.blocksync_url),
        ]
    ...
    def _repr_html_(self) -> str:
        ...
        # Start with common metadata (title with links, Object ID, Schema, Tags)
        html = self._metadata_repr_html_()

        # Build block model specific rows
        rows = [
        ...
        # Build the block model specific table
        table_rows = []
